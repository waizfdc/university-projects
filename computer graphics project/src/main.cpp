...
enum object_type {used = 0, arrow = 1, treasure = 2};

uint binarisation_threshold = 400;
...
int main(int argc, char **argv)
{
    ...
    try {
        ...
        src_image = load_image(argv[1]);
        // save_image(src_image, "output/1_given.bmp");
        to_print = src_image.deep_copy();

        img_corrected = linear_cor(src_image);
        // save_image(img_corrected, "output/2_linear.bmp");
        img_corrected = gamma_cor(img_corrected);
        // save_image(img_corrected, "output/3_add_gamma.bmp");
        img_corrected = img_corrected.unary_map(BoxFilterOp(4));
        // save_image(img_corrected, "output/4_blur.bmp");
        binar = binarise(img_corrected);
        // save_image(binar, "output/5_binar.bmp");
        ...
        // cerr << "Error lalka: " << s << endl;
        binarisation_threshold = 70;
        try {
            binar = binarise(img_corrected);
            // save_image(binar, "output/6_binar_new.bmp");
...
void make_dot(uint x, uint y, uint r, tuple<uint, uint, uint> color = make_tuple(255, 0, 0)) {
        for (uint i = x - r; i <= x + r; i++)
        for (uint j = y - r; j <= y + r; j++) {
            to_print(i, j) = color;
        }
}
...
void object::name_type(Matrix<int>& obj_matr) {
        ...
        // //отладочка. пометочка
        // make_dot(masscentr_x, masscentr_y, 2, make_tuple(255, 0, 0));

        // make_dot(distant_x, distant_y, 2, make_tuple(0, 255, 0));

        // make_dot(masscentr_x - dec * y + alfa * x, masscentr_y + dec * x + alfa * y, 1); //left top
        // make_dot(masscentr_x + dec * y + alfa * x, masscentr_y - dec * x + alfa * y, 1); //right top
        // make_dot(masscentr_x - inc * y - dec * alfa * x, masscentr_y + inc * x - dec * alfa * y, 1); //left bottom
        // make_dot(masscentr_x + inc * y - dec * alfa * x, masscentr_y - inc * x - dec * alfa * y, 1); //right bottom


        // to_print(masscentr_x - y + alfa * x, masscentr_y + x + alfa * y) = make_tuple(0, 255, 200);
        // to_print(masscentr_x + y + alfa * x, masscentr_y - x + alfa * y) = make_tuple(0, 255, 200);
        // to_print(masscentr_x - y - alfa * x, masscentr_y + x - alfa * y) = make_tuple(0, 255, 200);
        // to_print(masscentr_x + y - alfa * x, masscentr_y - x - alfa * y) = make_tuple(0, 255, 200);


        if (obj_matr(masscentr_x - dec * y + alfa * x, masscentr_y + dec * x + alfa * y) && 
            obj_matr(masscentr_x + dec * y + alfa * x, masscentr_y - dec * x + alfa * y) &&
            !obj_matr(masscentr_x - inc * y - dec * alfa * x, masscentr_y + inc * x - dec * alfa * y) && 
            !obj_matr(masscentr_x + inc * y - dec * alfa * x, masscentr_y - inc * x - dec * alfa * y))
            type = arrow;
        else
            type = treasure;
...
Image linear_cor( const Image& in) {
    ...
    for (uint i = 0; i < in.n_rows; i++)
        for (uint j = 0; j< in.n_cols; j++) {
            if (brightness_c(in(i, j)) < min) min = brightness_c(in(i, j));
            if (brightness_c(in(i, j)) > max) max = brightness_c(in(i, j));
        }
...
void make_path(const Image& img, std::vector<object> all, ofstream& fout) {
    ...
    //         for (uint i = object.min_x; i <= object.max_x; i++) {
    //             to_print(i, object.min_y) = make_tuple(255, 0, 0);
    //             to_print(i, object.max_y) = make_tuple(255, 0, 0);
    //         }
    //         if (object.type == arrow) {
    //             make_dot(object.masscentr_x, object.masscentr_y, 3, make_tuple(0, 0, 255));
    //             make_dot(object.distant_x, object.distant_y, 3, make_tuple(0, 255, 255));
    //         }
    //        for (uint i = object.min_y; i <= object.max_y; i++) {
    //             to_print(object.min_x, i) = make_tuple(255, 0, 0);
    //             to_print(object.max_x, i) = make_tuple(255, 0, 0);
    //         }
    // }
    // cout << "gonna save" << endl;
    // save_image(to_print, "8_toprint.bmp");
    // cout << "saved it!" << endl;
    {
        ...
        for (auto &obj : all) {
            if (obj.type == arrow) {
                ...
                if (r_over - g_over - b_over > 0) {
                    // make_dot(obj.masscentr_x, obj.masscentr_y, 10);
                    current = obj;
                    obj.type = used;
                    red_found = true;
                    break;
    ...
    if (!red_found) {
        string str("haven't found red arrow");
        throw str;
    }
    ...
    while (current.type == arrow) {
        ...
        while (!(cur_x > img.n_rows || cur_y > img.n_cols)) {
            auto c_obj = all.begin();
            for (auto obj = all.begin(); obj < all.end(); obj++) {
            ...
            if (found) {
                if (next_obj.type == used){
                    current.type = treasure;
                    break;
                }
        ...
        if (!found) {
            draw_line(to_print, current.masscentr_x, current.masscentr_y, cur_x - vect_x, cur_y - vect_y);

            //std::cout << "error/ not found" << endl;
            string er("missed destination");
            throw er;
            return;
        }
